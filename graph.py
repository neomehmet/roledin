...
def is_cable_include(hat_edge_ids, path_ids):
    ...
    for i in path_ids:
        if i == 13398981:
            ...
            print("hat bulundu ", i)
    """
    return any(i in hat_edge_ids for i in path_ids)
...
def get_path_ids(start_end_2_edge_id, path):
    ...
    for i in range(len(path) - 1):
        path_contains_id.append(start_end_2_edge_id[(path[i], path[i + 1])])
        ...
        for source_node_coord, ayirici_id, initial_conns in source_nodes:
            ...
            while queue:
                ...
                if paths is None:
                    ...
                    continue
                buffer_df, filter_param = self.create_temp_bufferV2(
                    ayirici_coord, paths, baglanti_start, baglanti_end
                )
                # print("filter param ", filter_param)

                buffer_df = self.filter_and_clear_bufferV2(
                    ayirici_coord, buffer_df, filter_param
                )
